...
{	
	...
	default:
		SerializationInfo.OutHeader += TEXT("/* Unsupported type: " + SATS::TypeToString(TypeEntry.Type) + " */\n");
		UE_LOG(LogTemp, Error, TEXT("[spacetime] Unsupported type: %s"), *SATS::TypeToString(TypeEntry.Type));
		break;
	}
...
{
	const auto &Tab = FSpacetimeConfig::TabString;
...
{	
	...
	if (SATS::IsBuiltIn(AlgebraicKind))
	{
		if (AlgebraicKind == SATS::EType::Array
		  | AlgebraicKind == SATS::EType::Map)
		{
	...
	OutSource += "// " + SATS::TypeToString(AlgebraicKind) + ":\n";
	for (auto i = 0; i < Indent; ++i) OutSource += FSpacetimeConfig::TabString;
