...
public class SpacetimeDBEditor : ModuleRules
{
    public SpacetimeDBEditor(ReadOnlyTargetRules target) : base(target)
    {
        ...
        */
        ...
        string SDKDepsRoot      = Path.Combine(SDKRoot, "lib/_deps");
        string SDKIncludeDir    = Path.Combine(SDKRoot, "include");
        
        // ssl_options
        PublicDefinitions.Add("SUPPORT_MAX_TLSv1_0=0");

        PublicIncludePaths.Add(SDKIncludeDir);
        PublicIncludePaths.Add(Path.Combine(SDKDepsRoot, "cpr-src/include"));
        PublicIncludePaths.Add(Path.Combine(SDKDepsRoot, "cpr-build/cpr_generated_includes"));
        PublicIncludePaths.Add(Path.Combine(SDKDepsRoot, "curl-src/include"));
        PublicIncludePaths.Add(Path.Combine(SDKDepsRoot, "json-src/include"));
        ...
        {
            PublicAdditionalLibraries.Add(Path.Combine(SDKRoot, "lib", "libSpacetimeDBSdk.a"));
            PublicAdditionalLibraries.Add(Path.Combine(SDKDepsRoot, "zlib-build", "libz.a"));
            PublicAdditionalLibraries.Add(Path.Combine(SDKDepsRoot, "curl-build/lib", "libcurl.a"));
            PublicAdditionalLibraries.Add(Path.Combine(SDKDepsRoot, "cpr-build/cpr", "libcpr.a"));
            
            PublicAdditionalLibraries.Add("ssl");    // -lssl
            PublicAdditionalLibraries.Add("crypto"); // -lcrypto
            PublicAdditionalLibraries.Add("curl");   // -lcurl
        ...
        else if (Target.Platform == UnrealTargetPlatform.Win64)
        {
            ...
            PublicAdditionalLibraries.Add(Path.Combine(SDKRoot, "lib", "libSpacetimeDBSdk.lib"));
        }
        // PublicAdditionalLibraries.Add("curl");      // -lcurl
        // PublicAdditionalLibraries.Add("ssl");       // -lssl
        // PublicAdditionalLibraries.Add("crypto");    // -lcrypto
